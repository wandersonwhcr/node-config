...
/**
 * Filtro para Elementos Únicos
 *
 * Função de _callback_ para filtro de conjunto de informações, retornando os valores sem repetição.
 * Utilizada para filtrar o conjunto de arquivos, carregando os arquivos sem repetição.
 *
 * @param  {string}   element  Elemento para Verificação
 * @param  {int}      position Posição do Elemento no Conjunto
 * @param  {string[]} elements Conjunto de Elementos de Entrada
 * @return bool       Confirmação de Elemento não Duplicado
 */
function unique(element, position, dataset) {
  return dataset.indexOf(element) === position;
}
...
Config.prototype.fetch = function fetch() {
  ...
  const filenames = fetchFilenames.call(self);
  // Limpeza: Únicos por Nome de Arquivo
  const elements = filenames.filter(unique);

  // Consulta
  const promises = elements.map(filename =>
    // Execução Assíncrona
...
Config.prototype.fetchSync = function fetchSync() {
  // Capturar Nomes de Arquivos
  const filenames = fetchFilenames.call(this);
  // Limpeza: Únicos por Nome de Arquivo
  const elements = filenames.filter(unique);

  // Execução
  return mergeElements(elements.map(filename => readAndBuildSync.call(this, filename)));
};
