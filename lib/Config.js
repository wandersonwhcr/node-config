 ...
 * algum problema seja encontrado, atira um objeto de erro.
 *
 * @param  {string} filename Nome do Arquivo
 * @throws {Error}  Erro Encontrado durante Processamento
 * @return {object} Objeto que Representa o Conteúdo do Arquivo
 */
Config.prototype._readSync = function (filename)
{
    return JSON.parse(this.getFs().readFileSync(filename));
};
...
Config.prototype.fetch = function ()
{
    ...
    this._fetchFilenames().forEach(function (filename) {
        ...
        promises.push(new Promise(function (resolve, reject) {
            ...
            try {
                // Sucesso!
                var element = {"filename": filename, "content": self._readSync(filename)};
                // Apresentação
                resolve(element);
            } catch (error) {
